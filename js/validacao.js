...
const tiposDeErros = ['valueMissing', 'typeMismatch', 'patternMismatch', 'customError'];
...
const mensagensDeErro = {
   ...
   cep: {
      ...
      customError: 'Não foi possível buscar o CEP digitado.'
   },
   logradouro: {
...
function recuperarCEP(input) {
   ...
   if (!input.validity.patternMismatch && !input.validity.valueMissing) {
      fetch(url, options).then(
         ...
         data => {
            if (data.erro) {
               input.setCustomValidity('Não foi possível buscar o CEP digitado.');
               return;
            }
